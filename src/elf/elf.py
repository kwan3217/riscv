...
def invert_syms(syms: dict[str, int]) -> dict[int, str]:
    """
    Create an inverse symbol table: address -> name.
    :param syms: Dict of symbol names to addresses (e.g., {"kfree": 0x80001234})
    :return: Dict of addresses to names (e.g., {0x80001234: "kfree"})
    Generated by Grok 3, works as-is
    """
    return {addr: name for name, addr in syms.items()}


def symbolic_addr(addr: int, inverse_syms: dict[int, str])->str:
    """
    Print an address as "symbol+offset" (e.g., "kfree+0x04" for 0x80001238).
    :param addr: Address to look up (e.g., 0x80001238)
    :param inverse_syms: Inverse symbol table (addr -> name)
    Generated by Grok 3, tuned to return the string instead of printing it and to not pass
    syms but only inverse_syms.
    """
    # Find nearest symbol before addr
    nearest_addr = max((a for a in inverse_syms.keys() if a <= addr), default=None)
    if nearest_addr is None:
        return ""  # No symbol—raw hex
    else:
        symbol = inverse_syms[nearest_addr]
